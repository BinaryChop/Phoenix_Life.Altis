...
switch (_code) do
{
	...
	case 57:
	{
		...
		if(_shift && {animationState player != "AovrPercMrunSrasWrflDf"} && {isTouchingGround player} && {stance player == "STAND"} && {speed player > 2} && {!life_is_arrested} && {(velocity player) select 2 < 2.0} && {time - jumpActionTime > 1.5}) then {
			...
			if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable["restrained",false]) && !life_istazed) then
			{
				...
				if("ItemRadio" in assignedItems cursorTarget) then {
     					cursorTarget removeweapon "ItemRadio";
      					hint "Das Handy von der Person wurde auf den Boden gelegt.";
      					_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);
				}else { 
					hint "Die Person die du niedergeschlagen hast, hat kein Handy!";
   				};
			};
		};
	};
	...
	// aussteigen und waffe holstern zuschließen
	case 18:{	
			if(!_shift && !_alt && _ctrlKey) then {
				[] call life_fnc_getOut;
			}
				else {
					if(_shift && !_alt && !_ctrlKey) then {
						[] call life_fnc_getIn;
	...
	case 48:
	{	_veh = vehicle player;
		if(_shift && playerSide in [west] && vehicle player != player && ((driver vehicle player) == player)) then 
		{
				systemChat "Lautsprecher eingeschaltet!";
				[[_veh],"life_fnc_durchsage1",nil,true] spawn life_fnc_MP;
				
		};
		if(_ctrlKey && playerSide in [west] && vehicle player != player && ((driver vehicle player) == player)) then
			{
				systemChat "Lautsprecher eingeschaltet!";
				[[_veh],"life_fnc_durchsage2",nil,true] spawn life_fnc_MP;
