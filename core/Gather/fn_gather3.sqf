...
_resourceZones = [
	"apple_1","apple_2","apple_3","apple_4","peaches_1","peaches_2","peaches_3","peaches_4","meth_1","lsd_1","heroin_1","cocaine_1","weed_1","Birnen_1","Birnen_2","Birnen_3","Birnen_4",
...
_musch = ["muschu","muschu","muschu","muschu","muschu","muschu","muschu","muschu","pearl"];
...
switch(true) do {
	case (_zone in ["apple_1","apple_2","apple_3","apple_4"]): {_gather = "apple"; _val = round(random 3)+5;};
	case (_zone in ["peaches_1","peaches_2","peaches_3"]): {_gather = "peach"; _val = round(random 3)+5;};
	case (_zone in ["Birnen_1","Birnen_2","Birnen_3","Birnen_4"]): {_gather = "Birnen"; _val = round(random 3)+5;};
	case (_zone in ["heroin_1"]): {_gather = "heroinu"; _val = 4;};
	case (_zone in ["cocaine_1"]): {_gather = "cocaine"; _val = 4;};
	case (_zone in ["weed_1"]): {_gather = "cannabis"; _val = 4;};
	case (_zone in ["food_1_4","food_1_7","food_1_2"]): {_gather = "trauu"; _val = round(random 5)+3;};
	case (_zone in ["food_1_14","food_1_15","food_1_12","food_1_16","food_1_13"]): {_gather = _random2; _val = 1;};
	case (_zone in ["food_2_1","food_2_2","food_2_3","food_2_4"]): {_gather = _random1; _val = round(random 10);};
	case (_zone in ["food_1_6","food_1_8","food_1_9"]): {_gather = "krabben"; _val = round(random 4)+10;};
	case (_zone in ["lsd_1"]): {_gather = "lsdu"; _val = round(random 4)+15;};
	case (_zone in ["wheat_1"]): {_gather = "weizen"; _val = round(random 10)+5;};
	//case (_zone in ["wool_1"]): {_gather = "wool"; _val = round(random 4);};
};
if (_gather == "pearl") then {_val = 3};
