...
if(playerSide == civilian) then {
		...
		if(typeOf (_curTarget) in ["C_Van_01_fuel_F","B_Truck_01_fuel_F","O_Truck_02_fuel_F","O_Truck_03_fuel_F"]) then {
			_Btn4 ctrlSetText localize "STR_vInAct_DevicePump";
			_Btn4 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_devicePump";
			if(!isNil {(_curTarget getVariable "mining")} OR !local _curTarget && {_curTarget in life_vehicles}) then {
